...
function InstallPackages() {
	...
	console.log(
		"Please wait... starting to install all required node packages using child process"
	);
	console.log(
		"If the bot can't install the package please install it manually"
	);
	try {
		child.execSync("npm i");
		console.log(
			'Install complete!, please run "node index" command again!'
		);
		process.exit();
	} catch (err) {
...
if (fs.existsSync("./node_modules")) {
	if (fs.existsSync("./node_modules/discord.js")) {
		const check = require("discord.js/package");
		if (Number(check.version.split(".")[0]) !== 14) {
			console.log(
				"Invalid Discord.JS Version!, Please use Discord.JS 14.x"
			);
			process.exit();
		}
	} else InstallPackages();
	if (fs.existsSync("./node_modules/axios")) {
		const check = require("axios/package");
	...
	if (fs.existsSync("./node_modules/chalk")) {
		const check = require("chalk/package");
...
if (client.config.token.startsWith("Put") || !client.config.token.length) {
	console.log(
		chalk.cyan("[PteroStats] ") +
			chalk.red("Error! Invalid Discord Bot Token")
	);
	process.exit();
}
