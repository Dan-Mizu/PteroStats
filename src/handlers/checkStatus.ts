...
export default function checkStatus({ client }) {
	function Embed({ node }) {
	...
	console.log(
		chalk.cyan("[PteroStats] ") + chalk.green("Getting nodes stats")
	);
			...
			.catch((error) => {
				if (error.response) {
					if (error.response.status === 403) {
						console.log(
							chalk.cyan("[PteroStats] ") +
								chalk.red("Error! Invalid apikey")
						);
						console.log(
							chalk.cyan("[PteroStats] ") +
								chalk.red(
						...
						console.log(
							chalk.cyan("[PteroStats] ") +
								chalk.red("Error! Invalid Panel URL")
						);
						console.log(
							chalk.cyan("[PteroStats] ") +
								chalk.red(
					...
					axios(
							...
							"/configuration",
						{
							...
							headers: {
								...
								"Content-Type": "application/json",
								Authorization:
									"Bearer " + client.config.panel.key,
							},
						}
					)
						.then((data) => {
							const body = {
								...
								location:
									node.attributes.relationships.location
										.attributes.short,
								allocations:
									node.attributes.relationships.allocations
										.data.length,
								maintenance: node.attributes.maintenance_mode,
								total_servers:
									node.attributes.relationships.servers.data
										.length,
								memory_min:
							...
							stats.then(() => {
								nodes.push(body);
								if (nodes.length === res.data.data.length)
									resolve();
							});
						})
						.catch(() => resolve());
