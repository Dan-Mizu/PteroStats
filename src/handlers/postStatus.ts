...
export default async function postStatus({ client, panel, nodes }) {
		...
		if (nodes.length !== 0) {
			nodes.forEach((data, i) => {
				if (!client.config.nodes_settings.blacklist.includes(data.id)) {
					...
					switch (client.config.nodes_settings.unit.toLowerCase()) {
						case "tb":
							description =
								...
								"\nMemory : " +
								Math.floor(
									data.memory_min / (1024 * 1000)
								).toLocaleString() +
								" TB / " +
								Math.floor(
									data.memory_max / (1024 * 1000)
								).toLocaleString() +
								" TB" +
								"\nDisk : " +
								Math.floor(
									data.disk_min / (1024 * 1000)
								).toLocaleString() +
								" TB / " +
								Math.floor(
									data.disk_max / (1024 * 1000)
								).toLocaleString() +
								" TB";
							break;
						case "gb":
							description =
								...
								"\nMemory : " +
								Math.floor(
									data.memory_min / 1024
								).toLocaleString() +
								" GB / " +
								Math.floor(
									data.memory_max / 1024
								).toLocaleString() +
								" GB" +
								"\nDisk : " +
								Math.floor(
						...
						case "percent":
							description =
								...
								"\nMemory : " +
								Math.floor(
									(data.memory_min / data.memory_max) * 100
								) +
								" %" +
								"\nDisk : " +
								Math.floor(
									(data.disk_min / data.disk_max) * 100
								) +
								" %";
							break;
						default:
							description =
								description +
								"\nMemory : " +
								data.memory_min.toLocaleString() +
								" MB / " +
								data.memory_max.toLocaleString() +
								" MB" +
								"\nDisk : " +
								data.disk_min.toLocaleString() +
								" MB / " +
								data.disk_max.toLocaleString() +
								" MB";
					}
					...
					if (
						nodes.length -
							client.config.nodes_settings.blacklist.length <
						1
					)
				...
				if (
					...
					if (
						...
						String(
							...
							String(
								String(
									messages.embeds[0].fields[0].value
								).split("\n")[2]
							).length - 1
						] !== "`"
					) {
						panel.total_users = String(
							String(messages.embeds[0].fields[0].value).split(
								"\n"
							)[2]
						).split("")[
	...
	stats.then(async () => {
		const format = await time(
			new Date(Date.now() + client.config.refresh * 1000),
			"R"
		);
		...
		if (client.config.button.enable) {
			...
			if (
				client.config.button.btn1.label.length !== 0 &&
				client.config.button.btn1.url.length !== 0
			) {
			...
			if (
				client.config.button.btn2.label.length !== 0 &&
				client.config.button.btn2.url.length !== 0
			) {
				button.addComponents(
					new ButtonBuilder()
			...
			if (
				client.config.button.btn3.label.length !== 0 &&
				client.config.button.btn3.url.length !== 0
			) {
				button.addComponents(
					new ButtonBuilder()
			...
			if (
				client.config.button.btn4.label.length !== 0 &&
				client.config.button.btn4.url.length !== 0
			) {
				button.addComponents(
					new ButtonBuilder()
			...
			if (
				client.config.button.btn5.label.length !== 0 &&
				client.config.button.btn5.url.length !== 0
			) {
				button.addComponents(
					new ButtonBuilder()
